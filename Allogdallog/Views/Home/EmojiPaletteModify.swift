//
//  EmojiPaletteModify.swift
//  Allogdallog
//
//  Created by 믕진희 on 11/26/24.
//

import SwiftUI

struct EmojiPaletteModify: View {
                    ...
                    ScrollView {
                        LazyVGrid(columns: columns, spacing: 10) {
                            ForEach(selectedEmojis, id:\.self) { emoji in
                                Button(action: {
                                    viewModel.clickedPost.todayText = emoji
                                }) {
                                    ZStack {
                                        Text(emoji)
                                            .font(.system(size: 32))
                                        if emoji == viewModel.clickedPost.todayText {
                                            Circle()
